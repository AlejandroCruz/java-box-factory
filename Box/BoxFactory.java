...
/* Program: BoxFactory
 * Create rectangles with (x,y) points and display.
 *
 * Compile/execute from CL:
 *  cd C:\JavaWorkspace\IvorHortons\Ch5\Box
 *  javac -cp "C:\Users\FISH-1\Documents\JavaWorkspace\IvorHortons\Ch5" BoxFactory.java
 *  java -cp "C:\Users\FISH-1\Documents\JavaWorkspace\IvorHortons\Ch5" Box.BoxFactory
 */

// User created classes
import Conversion.Metrico;
import Geometry.MyLine;
...
public class BoxFactory {
  ...
  public static void main(String[] args){
    ...
    MyPoint[]   points;                       // Array object jolds coordinates
    MyLine      line = new MyLine();          // Connects points and gives distance & area
    ...
    // Create MyPoint array and fill with points
     /* points = new MyPoint[coords.length];
    for(int i = 0; i < points.length; i++){
      points[i] = new MyPoint(coords[i][0], coords[i][1], i+1);
    } */
...
/*     System.out.println("\nThere are a total of " +
      MyPoint.getCount() + " points in the form (x,y):\n");
...
/*      for(int i = 0, j = 0; i < points.length; i += 4, j++){
      rectAngle[j] = new Rectangle(
        points[i], points[i+1], points[i+2], points[i+3], j+1);
    }
 */
